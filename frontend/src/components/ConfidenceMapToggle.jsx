...
const MODE = { FORECAST: "forecast", CONFIDENCE: "confidence" };

// Perceptually-reasonable, colorblind-safer AQI severity ramp
// (avoids pure red/green — see PRD UI/UX requirement 4).
const AQI_STOPS = [
...
export function getFillColor(point, mode) {
  ...
  if (mode === MODE.CONFIDENCE) {
    const width = Math.max(0, (point.upper_bound ?? point.value) - (point.lower_bound ?? point.value));
