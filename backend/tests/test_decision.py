...
from backend.decision.normalize import min_max_normalize, normalize_interventions
...
from backend.decision.optimizer import optimize_interventions, INTERVENTION_CATALOG
...
def test_constrained_greedy_solver_basic():
    ...
    res = optimize_interventions(budget=10000.0, inspectors=20, max_travel_time_hours=5.0)
    ...
    res_budget = optimize_interventions(budget=400.0, inspectors=10, max_travel_time_hours=5.0)
    ...
    res_inspectors = optimize_interventions(budget=10000.0, inspectors=1, max_travel_time_hours=5.0)
    ...
    res_time = optimize_interventions(budget=10000.0, inspectors=20, max_travel_time_hours=0.6)
...
def test_config_driven_weight_change():
    ...
    try:
        ...
        res_cost_heavy = optimize_interventions(budget=10000.0, inspectors=50, max_travel_time_hours=5.0)
        ...
        res_aqi_heavy = optimize_interventions(budget=10000.0, inspectors=50, max_travel_time_hours=5.0)
        ...
        assert order_cost_heavy != order_aqi_heavy or selected_cost_heavy[0]["score"] != selected_aqi_heavy[0]["score"]
