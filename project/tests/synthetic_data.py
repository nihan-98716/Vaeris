...
STATIONS = {
    "station_ito_delhi": {"lat": 28.6304, "lon": 77.2495, "land_use": "residential", "road_density": 0.85},
    "station_wazirpur_delhi": {"lat": 28.6996, "lon": 77.1652, "land_use": "industrial", "road_density": 0.40},
    "station_rohini_delhi": {"lat": 28.7495, "lon": 77.0565, "land_use": "residential", "road_density": 0.35},
    "station_rk_puram_delhi": {"lat": 28.5641, "lon": 77.1765, "land_use": "mixed", "road_density": 0.55},
    "station_anand_vihar_delhi": {"lat": 28.6469, "lon": 77.3151, "land_use": "mixed", "road_density": 0.75},
}
...
def generate_history(
    ...
    for station_id, meta in STATIONS.items():
        ...
        for i, ts in enumerate(timestamps):
            ...
            commute_bump = 25 * (np.exp(-((hour - 8) ** 2) / 8) + np.exp(-((hour - 19) ** 2) / 8))
            seasonal = 20 * np.sin(2 * np.pi * ts.dayofyear / 365)  # rough winter/summer swing
            noise = rng.normal(0, 8)
            aqi = max(20, base_aqi + commute_bump + seasonal + noise)

            wind_speed = max(0.2, rng.normal(2.5, 1.2))
            wind_direction = (280 + rng.normal(0, 25)) % 360  # prevailing NW-ish wind, roughly stubble-transport direction
            ...
            rows.append({
                ...
                "temperature": 18 + 10 * np.sin(2 * np.pi * (ts.dayofyear - 80) / 365) + rng.normal(0, 2),
                "humidity": min(95, max(20, 55 + rng.normal(0, 10))),
                "precipitation": max(0.0, rng.normal(0, 0.3)) if rng.random() < 0.05 else 0.0,
...
def _inject_stubble_burning_episode(df: pd.DataFrame) -> pd.DataFrame:
    ...
    fire_bearing_from_station = 315.0  # roughly NW, consistent with Punjab/Haryana stubble transport

    mask_window = df["timestamp"] >= episode_start
    df.loc[mask_window, "wind_direction"] = fire_bearing_from_station + np.random.normal(
        0, 5, size=mask_window.sum()
    )
    df.loc[mask_window, "wind_speed"] = np.random.uniform(2.0, 4.0, size=mask_window.sum())
    ...
    hours_into_window = (df.loc[mask_window, "timestamp"] - episode_start).dt.total_seconds() / 3600
...
def get_fire_events_for_signals(hours_ago_options=(3, 6, 9)) -> list:
    ...
    return [
        {"distance_km": 42.0, "bearing_deg": 315.0, "detected_hours_ago": hours_ago_options[0]},
        {"distance_km": 78.0, "bearing_deg": 320.0, "detected_hours_ago": hours_ago_options[1]},
    ]
